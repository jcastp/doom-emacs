...
We get out of the insert mode with just a key chord, instead of having to look for the Esc key.
...
#+begin_src emacs-lisp
  ;;(setq savehist-file "~/Nextcloud/config/.emacs.d/savehist")
  ;;(setq savehist-file (concat my-data-dir "savehist"))
...
** avy-goto-line keybindings
I want to map the avy-goto-line to the gsl keybinding.
...
** org-timeblock
I still don't know how to use this one.
...
#+begin_src emacs-lisp
(setq denote-directory "/home/jcastp/Nextcloud/personal/denote/")
#+end_src
...
** bibtex config
I will store the main bibliography file in my data directory:

*** citar
#+begin_src emacs-lisp :tangle packages.el
(package! citar
  :recipe (:host github :repo "emacs-citar/citar"))
#+end_src
#+begin_src emacs-lisp
(after! citar
(org-cite-global-bibliography
     (directory-files
      (concat (getenv "HOME") "/Nextcloud/config/.emacs.d/bibtex/") t
      "^[A-Z|a-z|0-9].+.bib$"))
...
** TODO paste in line after
#+begin_src emacs-lisp
(defun evil-paste-below ()
  "Create an empty line below, move cursor to it, and paste the contents of the yank ring."
  (interactive)
  ;;(evil-open-below 1)   ; Create an empty line below
  (forward-line)        ; Move cursor to the empty line
  (yank)               ; Paste the contents of the yank ring
  ;;(evil-open-below 1)   ; Create an empty line below
  (evil-force-normal-state) ; Force normal state
  )

;; Bind the function to a key, for example, to "SPC p" in Evil's normal state
(evil-define-key 'normal global-map (kbd "SPC P") 'evil-paste-below)
#+end_src
* Work environment
