...
We can now rotate the layout of the windows inside a frame. This can help us to reorganize the layout.
...
(after! org
  (setq org-capture-templates'(
  			     ...
  			     ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                               ...
                               ("P" "Personaje" entry
                                (file+headline "~/Nextcloud/escritura/retazos/ideas.org" "Ideas Personajes")
                                "* %i%?"
                                :empty-lines-after 1)
                               )
        )
...
** org-modern changes
I don't use all the org-modern features, so I disable some of them here:
#+begin_src emacs-lisp :tangle yes
(after! org-modern
...
** CANCELED org tags aligned to the right of the window
I want to have a dynamic tag alignment, dependent on the width of the current window.

CANCELED: Every time I change the window size, the tags realign, and that makes a change in the buffer, so I need to save it again. Canceling this and going back to the normal behavior.

#+begin_src emacs-lisp :tangle no
(defun my/org-adjust-tags-column ()
  "Dynamically set `org-tags-column` to align tags to the right edge of the window."
  (interactive)
  (when (derived-mode-p 'org-mode)
    ;; Calculate the window width and adjust some margins
    ;; choose between a fixed max width or the right margin of the window
    (setq org-tags-column (max -110 (- 0 (- (window-width) 15))))
    (org-align-tags t)))

;; Optionally, hook it to window size changes and Org Mode
(add-hook 'org-mode-hook #'my/org-adjust-tags-column)
(add-hook 'window-configuration-change-hook #'my/org-adjust-tags-column)
(add-hook 'window-size-change-functions
          (lambda (_frame) (my/org-adjust-tags-column)))
#+end_src
* Writing
** Default dictionary
...
** TODO Count words without orgmode properties
...
(defun my/count-words-ascii ()
  "Gets an org file, exports to ascii, and count the words.
    This is to avoid counting all the comments or superfluous words."
  (interactive)
  (save-excursion
    (let
        ;; org-ascii-export-as-ascii opens a new window by default, making it visible, and positions it in the frame. How can I avoid this?
        ((this-buffer (org-ascii-export-as-ascii))
         )
      (with-current-buffer this-buffer
        (let ((word-count (count-words (point-min) (point-max))))
          (message "Total words: %d" word-count)))
...
** TODO Insert edit snippet
I want to add edit snippets with a quick keybinding:

#+begin_src emacs-lisp :tangle yes
(defun my/insert-edit-snippet ()
  "Insert the edit snippet."
  (interactive)
  (yas-expand-snippet (yas-lookup-snippet "edición: comentarios para edición de texto")))

(global-set-key (kbd "C-c 3") 'my/insert-edit-snippet)
#+end_src

I want also to get all the occurrences in the file:
https://www.masteringemacs.org/article/searching-buffers-occur-mode

#+begin_src emacs-lisp :tangle yes
(defun my/view-my-edits ()
  "Check the occurrences of the edit notes."
  (interactive)
  (occur "# EDIT" 1))

(global-set-key (kbd "C-c 4") 'my/view-my-edits)
#+end_src
