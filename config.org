...
Based on the environment variables, it uses personal file or work one.

This uses a special file, not committed, with my personal information, located in the path below.
...
(if my-homeenvironment-p
  ...
  (load "~/Nextcloud/trabajo/work-info.el"))
#+end_src
...
** CANCELED org heading numbering
:LOGBOOK:
- State "CANCELED"   from              [2026-01-13 Tue 08:02] \\
  Removed from general org files.
:END:
...
I want to have different faces for different task status. Also, I have 2 workflows, and I want to have them visually distinct.

Items with backgrounds are my "active" states in each workflow—ONGOING and TOREVIEW—making them pop when scanning a document.

#+begin_src emacs-lisp :tangle yes
(after! org
  ;; ============================================
  ;; Task workflow faces (warmer palette)
  ;; ============================================
  (defface my/org-todo
  ...
  (defface my/org-waiting
    ...
    "Face for WAITING - blocked or on hold.")
  ...
  (defface my/org-done
    ...
    "Face for DONE - task completed.")

  (defface my/org-canceled
    '((((class color) (background light))
       :foreground "#70627f" :strike-through t)
      (((class color) (background dark))
       :foreground "#9f94a5" :strike-through t))
    "Face for CANCELED - task abandoned.")

  ;; ============================================
  ;; Writing workflow faces (cooler palette)
  ;; ============================================
  (defface my/org-towrite
    '((((class color) (background light))
       :foreground "#721045" :weight bold)
      (((class color) (background dark))
       :foreground "#feacd0" :weight bold))
    "Face for TOWRITE - content needs writing.")

  (defface my/org-toreview
    '((((class color) (background light))
       :foreground "#1f3f7f" :background "#dae5ff" :weight bold)
      (((class color) (background dark))
       :foreground "#82b0ec" :background "#132a4f" :weight bold))
    "Face for TOREVIEW - draft ready for review.")

  (defface my/org-redo
    '((((class color) (background light))
       :foreground "#a0132f" :background "#ffd5d5" :weight bold)
      (((class color) (background dark))
       :foreground "#ff9077" :background "#4a1418" :weight bold))
    "Face for REDO - needs rewriting.")

  (defface my/org-finished
    '((((class color) (background light))
       :foreground "#006800" :weight bold)
      (((class color) (background dark))
       :foreground "#44bc44" :weight bold))
    "Face for FINISHED - writing complete.")

  (defface my/org-purge
    '((((class color) (background light))
       :foreground "#70627f" :strike-through t)
      (((class color) (background dark))
       :foreground "#9f94a5" :strike-through t))
    "Face for PURGE - content to be removed.")
  ...
  (setq org-todo-keyword-faces
        '(;; Task workflow
          ("TODO"     . my/org-todo)
          ("ONGOING"  . my/org-ongoing)
          ("WAITING"  . my/org-waiting)
          ("DONE"     . my/org-done)
          ("CANCELED" . my/org-canceled)
          ;; Writing workflow
          ("TOWRITE"  . my/org-towrite)
          ("TOREVIEW" . my/org-toreview)
          ("REDO"     . my/org-redo)
          ("FINISHED" . my/org-finished)
          ("PURGE"    . my/org-purge))))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
;; this is needed to load the org-clock variables before using this function
(require 'org-clock)
...
(map! :leader
      (:prefix-map ("d" . "personal prefix")
                   ...
                   :desc "center to screen"  "c"  #'my/centered-mode
                   )
      )
#+end_src
