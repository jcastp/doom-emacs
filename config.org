...
** CANCELED testing ~C-q~ prefix
I use one of the free prefixes, SPC d, to map my own functions, the ones I will be using more.
#+begin_src emacs-lisp :tangle no
;; First, unbind C-q (it's normally `quoted-insert`)
(map! "C-q" nil)

;; Define your prefix with submenus
(map! :prefix "C-q"
      :desc "My menu" "" nil  ; optional: gives the prefix a which-key label

      ;; Direct commands
      "i" #'yas-insert-snippet
      "s" #'save-buffer
      "z" #'quoted-insert  ; recover the original C-q if you want

      ;; Nested submenu for "files"
      (:prefix ("f" . "files")
               "f" #'find-file
               "r" #'recentf-open-files
               "s" #'save-buffer)

      ;; Nested submenu for "buffers"
      (:prefix ("b" . "buffers")
               "b" #'switch-to-buffer
      ...
      ;; Quick file access functions
      (:prefix ("q" . "quick file access")
               "c" (cons "personal config" (lambda () (interactive) (find-file "~/.doom.d/config.org")))
               "k" (cons "emacs keys" (lambda () (interactive) (find-file "~/Nextcloud/tech/org-keys.org")))
               "t" (cons "tasks file" (lambda () (interactive) (find-file "~/Nextcloud/agenda/tasks.org")))
               )

      ;; Quick file access functions
      (:prefix ("t" . "themes and fonts")
...
#+begin_src emacs-lisp :tangle yes
(map! :leader
      :desc "outline"   "o o" #'consult-outline)
#+end_src
